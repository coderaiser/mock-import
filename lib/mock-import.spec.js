...
test('mock-import: mockImport: transformSource: no change', async (t) => {
    ...
    const defaultTransformSource = stub().resolves('no change');
...
test('mock-import: mockImport: load', async (t) => {
    ...
    const defaultLoad = stub().resolves({
        format: 'module',
        source: await readFile(join(__dirname, name), 'utf8'),
    });
...
test('mock-import: mockImport: transformSource: traceImport: reImport: nested', async (t) => {
    ...
    const expected = [
        ['parse', 'parser.js:4', ['const a = 5']],
        ['tokenize', 'tokenizer.js:2', ['parser call', 'const a = 5']],
    ];
...
test('mockImport: disableNestedImports', (t) => {
    disableNestedImports(global.__mockImportNested);
